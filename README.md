...
- Packer запекаю образ с предустановленным docker, docker-compose и docker-compose.yml для сборки и запуска контейнера с gitlab ci
...
- Ansible произвожу модификацию docker-compose файла с изменением значения <YOUR-VM-IP> на текущий IP адрес ansible_host, запускаю docker-compose  разворачивая и запуская конейнер с GitLab CI
